...
> **단일 빌드 (안정판):** STS2 **`v0.107`** 부터 공개 안정판 브랜치가 구
> 베타 빌드가 대응하던 API로 합류해, 안정판/베타 구분이 사라졌습니다 — 버전마다
> 하나의 빌드(`Sts2UndoMod-vX.Y.Z.zip`)만 배포됩니다. **STS2 `v0.107.1`**
> (기본 브랜치) 기준으로 빌드·테스트했습니다. STS2는 얼리 액세스라, 향후 게임
> 업데이트로 깨질 수 있으며 그때는 모드 재빌드가 필요합니다. 패치 후 되돌리기가
> 안 되면 모드 업데이트를 확인하세요.
...
- **v0.0.14** — 적이 기절형 "가짜 죽음" 상태에 들어갈 때(예: 폭포 거인이 0 HP에서
  무한 HP로 전환되며 `die` 애니메이션을 반복) 언두가 영구 비활성화되어 `Z`가
  무반응이 되던 버그 수정. 정착된 `*_loop` 자세를 진행 중인 사망 애니로 오판해
  남은 전투 내내 언두를 막던 문제. ([#3](../../issues/3))
